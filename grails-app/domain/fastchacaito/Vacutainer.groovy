...
class Vacutainer {
    ...
    static avaliableVacutainers(){
        ...
        def query= Vacutainer.executeQuery("from Vacutainer as vac WHERE vac.vacutainerType='Mesoterapia' AND vac.id not in (select v.id from Vacutainer as v,SesionControl as s WHERE v.id=s.vacutainer OR v.id=s.vacutainer2)")
        return query
    }
    
    static vacutainersBalance(){
